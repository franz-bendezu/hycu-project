...
def facade_counts_from_inference(result: dict) -> tuple[int, int]:
    ...
    door_count = component_quantity_by_tokens(result, door_tokens)
    drawer_count = component_quantity_by_tokens(result, drawer_tokens)
    generic_front_count = component_quantity_by_tokens(result, generic_front_tokens)

    if door_count == 0:
        door_count = raw_detection_count_by_tokens(result, door_tokens)
    if drawer_count == 0:
        drawer_count = raw_detection_count_by_tokens(result, drawer_tokens)
    if generic_front_count == 0:
        generic_front_count = raw_detection_count_by_tokens(result, generic_front_tokens)
...
def facade_evidence_from_inference(result: dict) -> dict[str, int]:
    ...
    explicit_door_count = component_quantity_by_tokens(result, door_tokens)
    explicit_drawer_count = component_quantity_by_tokens(result, drawer_tokens)
    generic_front_count = component_quantity_by_tokens(result, generic_front_tokens)

    raw_door_count = raw_detection_count_by_tokens(result, door_tokens)
    raw_drawer_count = raw_detection_count_by_tokens(result, drawer_tokens)
    raw_generic_front_count = raw_detection_count_by_tokens(result, generic_front_tokens)
...
def divider_count_from_inference(
    ...
    divider_count = component_quantity_by_tokens(result, divider_tokens)
    if divider_count == 0:
        divider_count = raw_detection_count_by_tokens(result, divider_tokens)
...
def has_divider_evidence(result: dict) -> bool:
    divider_tokens = ("divider", "partition", "vertical")
    component_hits = component_quantity_by_tokens(result, divider_tokens)
    if component_hits > 0:
        return True
    return raw_detection_count_by_tokens(result, divider_tokens) > 0
...
def infer_model_type(result: dict, fallback: str = "cabinet") -> str:
    from_components = inferred_type_from_components(result)
    if from_components:
        return from_components

    from_raw = inferred_type_from_raw_detections(result)
    if from_raw:
        return from_raw

    detected = inferred_type_from_detected_type(result.get("detected_type"))
    if detected:
        return detected
...
def shelf_count_from_inference(result: dict, inferred_type: str) -> int:
    shelf_qty = component_quantity(result, "shelf_panel")
    if shelf_qty > 0:
        return shelf_qty
    return shelf_count_from_detected_type(inferred_type)
...
def layout_axis_from_raw_detections(result: dict, tokens: tuple[str, ...], axis: str) -> list[float]:
    ...
    for evidence in image_results:
        ...
        if not isinstance(raw_detections, list):
            continue
        width_px = safe_float(evidence.get("width_px"), default=1.0)
        height_px = safe_float(evidence.get("height_px"), default=1.0)
        ...
        for detection in raw_detections:
            ...
            if not label or not any(token in label for token in tokens):
                continue
            center = normalized_center_from_box(detection.get("box"), width_px, height_px)
            if center is None:
                continue
...
def layout_axis_from_components(result: dict, tokens: tuple[str, ...], axis: str) -> list[float]:
    ...
    for component in components:
        ...
        center = normalized_center_from_box(component.get("box_corners"), 1.0, 1.0)
        if center is None:
            continue
...
def apply_inference_layout_to_joints(model: ProjectModel, result: dict) -> None:
    ...
    for kind_name, tokens, axis in kind_tokens_axis:
        ...
        anchors = layout_axis_from_raw_detections(result, tokens=tokens, axis=axis)
        if not anchors:
            anchors = layout_axis_from_components(result, tokens=tokens, axis=axis)
        fitted = fit_anchor_count(anchors, len(child_ids))
        if not fitted:
            continue
